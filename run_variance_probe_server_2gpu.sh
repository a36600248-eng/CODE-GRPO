...
if [ $# -lt 1 ]; then
  echo "Usage: bash run_variance_probe_server_2gpu.sh <seed> [port]"
  exit 1
fi

SEED=$1
PORT=${2:-8012}
ORCH_LOG=/root/autodl-tmp/CODE-GRPO/trl/variance_probe_orchestrator_${PORT}.log
...
log "Variance probe orchestrator starting: seed=${SEED} base_port=${PORT}"
...
CONFIG_PATH=configs/comparison/server_2gpu_smoke/codegrpo_variance_probe.yaml
...
cleanup() {
  if [ -n "${SERVER_PID:-}" ] && kill -0 "$SERVER_PID" 2>/dev/null; then
    log "Stopping variance-probe vLLM server process group: ${SERVER_PID}"
...
start_server() {
  ...
  local server_log="/root/autodl-tmp/CODE-GRPO/trl/vllm_server_variance_probe_${port}.log"
  ...
  log "Starting variance-probe vLLM server on GPU0, port=${port}"
...
run_train() {
  local port="$1"
  local group_port="$((port + 40000))"
  log "Running variance probe config: ${CONFIG_PATH} (port=${port}, group_port=${group_port})"
...
log "Variance probe orchestrator finished successfully"

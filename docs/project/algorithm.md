...
为诊断 `K=2` 下“哪些题真正提供了代码梯度”，实现中额外维护了**题目级轻量信号状态**，但该状态**不参与 reward 公式**，仅用于日志/后续采样策略试验。

当前题目级信号分类口径（与实现一致）：

1. 只统计**主代码样本**，不统计 logic/exec 审阅样本。
2. 必须按 **sibling group**（同一 `prompt_text` 的主代码样本）逐组判定，不能把整题所有代码样本做一次 flat 聚合。
3. 题目级结果由各组汇总得到：

```python
easy_solved:
    所有 sibling group 都 all-pass

useful_signal:
    至少一个 sibling group 中存在 abs(A_code) > eps

undiff_unsolved:
    不属于 easy_solved
    且没有任何 sibling group 产生 useful_signal
    且至少一个 sibling group 满足：
        not all-pass
        and same R_code
        and same pass_rate
```
...
* 这里的诊断目标是识别“未解决且无区分”的题，而不是修改主训练目标。
* 不能直接用整题 `max(R_code)-min(R_code)` 判断是否有信号，因为多轮树搜索下不同 sibling group 的绝对 `R_code` 可能不同，但每组内部仍可能完全打平。
* 相关日志项包括：`signal/undiff_unsolved_rate`、`signal/easy_solved_rate`、`signal/useful_signal_rate`。
...
* `signal_weighted_sampling`（当前仅保留为 dataloader-build-time snapshot 占位开关，默认关闭）
...
关于 `signal_weighted_sampling` 的实现现状（与代码一致）：

* 当前 sampler 权重只在 `get_train_dataloader()` 构建时计算一次。
* HF Trainer 默认不会每个 epoch 自动重建 train dataloader，因此这些权重**不会按 epoch 刷新**。
* 训练起始时题目级 signal state 为空，因此若此时启用该开关，权重通常全为 `1.0`，等价于均匀采样。
* 因此该开关目前只作为后续“epoch 级 sampler 重建”方案的占位项；在真正实现重建 hook 前，推荐保持关闭。

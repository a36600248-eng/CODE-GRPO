...
if [ $# -lt 1 ]; then
  echo "Usage: bash run_soft_grpo_train_eval_server_2gpu.sh <seed> [port]"
  exit 1
fi

SEED=$1
PORT=${2:-8017}
ORCH_LOG=/root/autodl-tmp/CODE-GRPO/trl/soft_grpo_train_eval_orchestrator_${PORT}.log
...
log "Soft-reward GRPO train+eval orchestrator starting: seed=${SEED} base_port=${PORT}"
...
TRAIN_OUTPUT_DIR=/root/autodl-tmp/CODE-GRPO/trl/runs_single_round_soft_grpo_seed${SEED}
EVAL_OUTPUT_DIR=/root/autodl-tmp/CODE-GRPO/trl/runs_single_round_soft_grpo_eval_seed${SEED}
...
cleanup() {
  if [ -n "${SERVER_PID:-}" ] && kill -0 "$SERVER_PID" 2>/dev/null; then
    log "Stopping soft-reward GRPO vLLM server process group: ${SERVER_PID}"
...
start_server() {
  ...
  log "Starting soft-reward GRPO vLLM server on GPU0, port=${port}"
...
run_train() {
  ...
  log "Running soft-reward GRPO train"
  CUDA_VISIBLE_DEVICES=1 python -m trl.cli.main code_grpo     --config "${TRAIN_CONFIG}"     --seed "${SEED}"     --data_seed "${SEED}"     --output_dir "${TRAIN_OUTPUT_DIR}"     --vllm_server_base_url "http://127.0.0.1:${port}"     --vllm_group_port "${group_port}"
}
...
log "Soft-reward GRPO train+eval orchestrator finished successfully"

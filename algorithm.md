...
* 执行审计样本（`<REASON> + <EXEC_PREDICTION>`）：`code_token_mask=全0`，`reason_token_mask=全1`，即 `A_reason` 只作用于执行审计输出 token
* 逻辑审计样本不参与反向传播（只用于计算 `R_soft`）
...
```python
if logic_format_ok:
    logic_score = clamp01(logic_correct)
else:
    logic_score = clamp01(logic_correct * (1 - format_penalty_logic))
```

* 当 `logic_correct in {0,1}` 时，上述实现与 `max(0, logic_correct - format_penalty_logic)` 等价。
...
```python
R_code = pass_rate + lambda_soft * (1 - pass_rate) * R_soft_effective
```
...
* 在 `R_soft_effective in [0,1]` 且 `lambda_soft <= 1` 时，`R_code <= 1`

### 9.4 soft reward 生效门控（与代码一致）

实现里先计算原始 `R_soft`，再做门控：
...
* 节点可以有 `R_soft_raw`，但在语法错误或缺少顶层 `solve` 入口时，soft 不会加到 `R_code` 上。
* 这样可抑制 reward hacking（例如输出无效代码却靠逻辑分支“刷分”）。
...
* 执行分支也有格式惩罚 `format_penalty_exec`；格式不合法时使用同样的乘法扣减并截断到 `[0,1]`：

```python
if exec_format_ok:
    exec_score = clamp01(exec_correct)
else:
    exec_score = clamp01(exec_correct * (1 - format_penalty_exec))
```
...
* 逻辑分支会产生 `R_soft_raw`，其计算不依赖代码可执行性；但 `R_code` 只使用门控后的 `R_soft_effective`（见 9.4）。
...
* `format_penalty_logic, format_penalty_exec`
...
4. **soft reward 来自逻辑推理分支**：`<LOGIC_PREDICTION>` 要与标准答案比较；`R_code = pass_rate + lambda_soft * (1 - pass_rate) * R_soft_effective`

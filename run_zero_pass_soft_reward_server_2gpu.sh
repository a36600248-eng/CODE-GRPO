...
if [ $# -lt 1 ]; then
  echo "Usage: bash run_zero_pass_soft_reward_server_2gpu.sh <seed> [port]"
  exit 1
fi

SEED=$1
PORT=${2:-8014}
ORCH_LOG=/root/autodl-tmp/CODE-GRPO/trl/zero_pass_soft_reward_orchestrator_${PORT}.log
...
log "Zero-pass soft reward orchestrator starting: seed=${SEED} base_port=${PORT}"
...
log "Clearing stale vLLM / training processes before zero-pass soft reward run"
...
CONFIG_PATH=configs/comparison/server_2gpu_smoke/codegrpo_single_round_zero_pass_soft_reward.yaml
...
cleanup() {
  if [ -n "${SERVER_PID:-}" ] && kill -0 "$SERVER_PID" 2>/dev/null; then
    log "Stopping zero-pass soft reward vLLM server process group: ${SERVER_PID}"
...
start_server() {
  ...
  local server_log="/root/autodl-tmp/CODE-GRPO/trl/vllm_server_zero_pass_soft_reward_${port}.log"
  ...
  log "Starting zero-pass soft reward vLLM server on GPU0, port=${port}"
...
run_train() {
  local port="$1"
  local group_port="$((port + 40000))"
  log "Running zero-pass soft reward config: ${CONFIG_PATH} (port=${port}, group_port=${group_port})"
...
log "Zero-pass soft reward orchestrator finished successfully"

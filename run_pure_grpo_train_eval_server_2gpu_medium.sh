...
if [ $# -lt 1 ]; then
  echo "Usage: bash run_pure_grpo_train_eval_server_2gpu_medium.sh <seed> [port]"
  exit 1
fi

SEED=$1
PORT=${2:-8024}
ORCH_LOG=/root/autodl-tmp/CODE-GRPO/trl/pure_grpo_train_eval_medium_orchestrator_${PORT}.log
...
log "Pure GRPO medium train+eval orchestrator starting: seed=${SEED} base_port=${PORT}"
...
TRAIN_CONFIG=configs/comparison/server_2gpu/codegrpo_single_round_pure_grpo_mbpp_medium.yaml
EVAL_CONFIG=configs/comparison/server_2gpu/raw_qwen7b_eval_mbpp_full.yaml
TRAIN_OUTPUT_DIR=/root/autodl-tmp/CODE-GRPO/trl/runs_single_round_pure_grpo_medium_seed${SEED}
EVAL_OUTPUT_DIR=/root/autodl-tmp/CODE-GRPO/trl/runs_single_round_pure_grpo_medium_eval_seed${SEED}
...
cleanup() {
  if [ -n "${SERVER_PID:-}" ] && kill -0 "$SERVER_PID" 2>/dev/null; then
    log "Stopping pure GRPO medium vLLM server process group: ${SERVER_PID}"
...
start_server() {
  local port="$1"
  local server_log="/root/autodl-tmp/CODE-GRPO/trl/vllm_server_pure_grpo_medium_${port}.log"

  log "Starting pure GRPO medium vLLM server on GPU0, port=${port}"
...
run_train() {
  ...
  log "Running pure GRPO medium train"
...
run_eval() {
  ...
  log "Running pure GRPO medium eval on trained adapter"
  CUDA_VISIBLE_DEVICES=1 python -m trl.cli.main code_grpo_eval \
    ...
    --vllm_server_base_url "http://127.0.0.1:${port}" \
    --vllm_group_port "${group_port}"
  log "Pure GRPO medium eval finished. Train adapter: ${trained_adapter_dir} Eval dir: ${EVAL_OUTPUT_DIR}"
}
...
log "Pure GRPO medium train+eval orchestrator finished successfully"

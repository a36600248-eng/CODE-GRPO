...
if [ $# -lt 1 ]; then
  echo "Usage: bash run_soft_grpo_train_eval_server_2gpu_mixed500_short.sh <seed> [port]"
  exit 1
fi

SEED=$1
PORT=${2:-8042}
ORCH_LOG=/root/autodl-tmp/CODE-GRPO/trl/soft_grpo_train_eval_mixed500_short_orchestrator_${PORT}.log
...
TRAIN_CONFIG=configs/comparison/server_2gpu/codegrpo_single_round_zero_pass_soft_reward_mixed500_short.yaml
EVAL_CONFIG=configs/comparison/server_2gpu/raw_qwen7b_eval_mixed500.yaml
TRAIN_OUTPUT_DIR=/root/autodl-tmp/CODE-GRPO/trl/runs_single_round_soft_grpo_mixed500_short_seed${SEED}
EVAL_OUTPUT_DIR=/root/autodl-tmp/CODE-GRPO/trl/runs_single_round_soft_grpo_mixed500_short_eval_seed${SEED}
...
start_server() {
  local port="$1"
  local server_log="/root/autodl-tmp/CODE-GRPO/trl/vllm_server_soft_grpo_mixed500_short_${port}.log"
